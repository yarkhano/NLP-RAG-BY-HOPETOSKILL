# models.py — Pydantic models that define the shape of every API request and response.
# FastAPI reads these classes and:
#   1. Validates incoming data automatically (wrong type → 422 error before your code runs)
#   2. Generates the Swagger /docs UI automatically (no extra work needed)
# You already know Pydantic from LangGraph's TypedDict — same idea, but for HTTP.

from typing import List, Optional
from pydantic import BaseModel, Field


# ── What a single source reference looks like ─────────────────────────────────
